...
| The bundle gate has **two** budgets since #331 (`web/scripts/check-bundle-size.mjs:36-37`): **entry** 134.9 KB against 150 KB — the entry chunk plus its transitive *static* imports, what a first paint costs, the tight ratchet — and **total** 373.5 KB against 400 KB, every emitted asset, a loose ceiling (— measured 2026-09-03, after #329 removed mathjs and total was ratcheted 450 → 400). The shells and all fourteen desktop tabs are lazy, so most chunks are not first-paint. `npm run size` runs inside **Build**. `CLAUDE.md:296-320` states the same figures; the two agree as of that date. Re-measure rather than quoting any doc — figures here have drifted before (#319). | Be surprised by a red Build that is not a compile error. |
| **A PR that raises `total` while cutting `entry` is a win, not a regression** — splitting always adds chunk preamble and duplicated shared code. Read the entry row of the bundle comment first; the old single total-bytes gate is exactly what #331 removed for reporting the 66% first-paint cut it measured as a failure — with main then at 399.2 KB of 400 KB, 0.8 KB spare. | Ask a PR to undo a code-split because one number went up. |

## When you push a fix

| Fact | Without it you would |
|---|---|
| On a **Dependabot** PR never use `gh pr update-branch` — a `GITHUB_TOKEN` push never re-triggers required checks, so auto-merge stalls forever (`dependabot-maintenance.yml:8-11`). Comment `@dependabot rebase` — **from an actor with push access**: Dependabot refuses the command from `github-actions[bot]`, which is why `rebase-behind` posts it with a GitHub App token (`dependabot-maintenance.yml:13-20`, #351; until then the job had been a no-op). An agent's own `GITHUB_TOKEN` comment will be refused the same way. Actions is also barred from approving here; the attempt broke the 2026-07-02 run on PR #124 (`dependabot-auto-merge.yml:7-9`). | Permanently stall an armed auto-merge. |
