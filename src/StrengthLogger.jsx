...
function mountStrengthLogger(root) {
  ...
  function e1rm(w,r){ if(!w||!r) return null; return Math.round(w*(1+r/30)); }
  const cap = s => s? s.charAt(0).toUpperCase()+s.slice(1):'';

  /* ---------- state ---------- */
  let USER=null;
  let active=null;
  let restTimer=null, restRemain=0, restEi=null;
  let EXPREF=null;
  ...
  async function loadPrefs(){
    ...
    EXPREF={};
    const {data}=await sb.from('exercise_prefs').select('exercise_id,rest_seconds');
    (data||[]).forEach(r=>{ if(r.rest_seconds!=null) EXPREF[r.exercise_id]=r.rest_seconds; });
    return EXPREF;
  }
  function restPref(id,fallback){ return (EXPREF&&id&&EXPREF[id]!=null)?EXPREF[id]:fallback; }
  function saveRestPref(id,sec){
    if(!id||!USER) return;
    if(!EXPREF) EXPREF={}; EXPREF[id]=sec;
    sb.from('exercise_prefs').upsert(
      { user_id:USER.id, exercise_id:id, rest_seconds:sec, updated_at:new Date().toISOString() },
      { onConflict:'user_id,exercise_id' }
    ).then(({error})=>{ if(error) console.warn('pref save',error.message); });
  }

  /* ---------- nav ---------- */
  root.querySelectorAll('nav button').forEach(b=>b.onclick=()=>show(b.dataset.tab));
  function setNav(tab){root.querySelectorAll('nav button').forEach(b=>b.classList.toggle('active',b.dataset.tab===tab));}
  function show(view){
    $('#homeView').classList.toggle('hidden',view!=='home');
    $('#historyView').classList.toggle('hidden',view!=='history');
    $('#workoutView').classList.toggle('hidden',view!=='workout');
    if(view==='home'){ setNav('home'); loadHome(); }
    if(view==='history'){ setNav('history'); loadHistory(); }
  }

  /* ---------- HOME ---------- */
  async function loadHome(){
    const v=$('#homeView'); v.innerHTML='<div class="empty"><span class="spin"></span></div>';
    loadPrefs();
    const {data:assigns}=await sb.from('workout_assignments')
      .select('*, templates(*)')
      .in('status',['pending','in_progress'])
      .lte('assigned_date',todayISO())
      .order('assigned_date',{ascending:true});
    const {data:tpls}=await sb.from('templates').select('*').eq('is_archived',false)
      .order('origin',{ascending:true}).order('name',{ascending:true});
    v.innerHTML='';
    if(active){
      ...
      const b=el('button','btn good','Resume'); b.style.marginTop='10px';
      b.onclick=()=>show('workout'); c.appendChild(b); v.appendChild(c);
    }
    const addCard=el('div','card');
    addCard.innerHTML='<div style="font-weight:800;font-size:17px;margin-bottom:3px">Strength training</div>'
    ...
    addBtn.onclick=startCustom; addCard.appendChild(addBtn); v.appendChild(addCard);
    ...
    if(assigns&&assigns.length){
      assigns.forEach(a=>{
        ...
        const b=el('button','btn','Start workout'); b.style.marginTop='12px';
        b.onclick=(e)=>{e.stopPropagation();startFromTemplate(t,{origin:'coach',assignment:a});};
        c.appendChild(b); v.appendChild(c);
      });
    } else {
      v.appendChild(el('div','empty','No coach-prescribed workouts due. Nice — pick a template or go custom.'));
    }
    ...
    } else { v.appendChild(el('div','empty','No templates yet.')); }
  }
  ...
  async function startFromTemplate(tpl,opts){
    opts=opts||{};
    await loadPrefs();
    const {data:tex}=await sb.from('template_exercises').select('*').eq('template_id',tpl.id).order('position');
    const list=(tex||[]);
    const lastPerf=await fetchLastPerf(list.map(te=>te.exercise_id).filter(Boolean));
    const exercises=list.map(te=>{
      let plan=Array.isArray(te.set_plan)&&te.set_plan.length? te.set_plan
        : Array.from({length:te.target_sets||3}).map(()=>({
            weight_kg:te.target_weight_kg, reps:te.target_reps, rpe:te.target_rpe, type:'work'}));
      return {
        exercise_id:te.exercise_id, exercise_name:te.exercise_name,
        target_reps:te.target_reps, target_rpe:te.target_rpe, target_sets:te.target_sets,
        rest_seconds:restPref(te.exercise_id, te.rest_seconds||120), notes:te.notes,
        ...
        sets: plan.map(p=>({
          ...
          warmup: p.type==='warmup', done:false,
          sug:{ weight:p.weight_kg, reps:p.reps, rpe:p.rpe } }))
      };
    });
    beginWorkout({
      label: tpl.name, session_type: tpl.session_type||'Strength',
      origin: opts.origin||'template', template_id: tpl.id,
      assignment: opts.assignment||null, exercises
    });
  }
  async function fetchLastPerf(ids){
    const out={}; if(!ids||!ids.length) return out;
    const {data}=await sb.from('exercise_last_performance').select('*').in('exercise_id',ids);
    (data||[]).forEach(r=>{ (out[r.exercise_id]=out[r.exercise_id]||{date:r.workout_date,sets:[]})
      .sets.push({i:r.set_index,w:r.weight_kg,reps:r.reps,rpe:r.rpe,e1rm:r.e1rm_kg}); });
    Object.values(out).forEach(o=>o.sets.sort((a,b)=>a.i-b.i));
    return out;
  }
  function startCustom(){
    beginWorkout({ label:'Custom workout', session_type:'Strength', origin:'custom',
      template_id:null, assignment:null, exercises:[] });
  }
  async function beginWorkout(w){
    const ins={ label:w.label, session_type:w.session_type, status:'in_progress',
      origin:w.origin, template_id:w.template_id,
      assignment_id:w.assignment?w.assignment.id:null, started_at:new Date().toISOString() };
    const {data,error}=await sb.from('strength_workouts').insert(ins).select().single();
    if(error){toast('Could not start: '+error.message);return;}
    w.id=data.id; w.started=Date.now();
    if(w.assignment){ sb.from('workout_assignments').update({status:'in_progress',workout_id:w.id}).eq('id',w.assignment.id).then(()=>{}); }
    active=w; renderWorkout(); show('workout');
  }

  /* ---------- ACTIVE WORKOUT RENDER ---------- */
  function renderWorkout(){
    const v=$('#workoutView'); v.innerHTML='';
    ...
    const add=el('button','btn sec','+ Add exercise'); add.style.marginBottom='14px';
    add.onclick=()=>openPicker(); v.appendChild(add);
    const fin=el('button','btn good','Finish & save workout'); fin.onclick=()=>finishWorkout(fin);
    v.appendChild(fin);
    const cancel=el('button','btn danger','Discard workout'); cancel.style.marginTop='10px';
    cancel.onclick=discardWorkout; v.appendChild(cancel);
    tickElapsed();
  }
  function renderExercise(ex,ei){
    ...
    const tgt=[]; if(ex.target_sets&&ex.target_reps) tgt.push(`${ex.target_sets}×${ex.target_reps}`);
    ...
    if(ex.exercise_id){
      const db=el('button','demo-btn','◐ Demo'); db.title='Movement & muscles worked';
      db.onclick=()=>openExerciseDemo(ex.exercise_id,ex.exercise_name);
      rmv.appendChild(db);
    }
    const rb=el('button','btn xs sec','✕'); rb.style.flex='0 0 auto';
    rb.onclick=()=>{active.exercises.splice(ei,1);renderWorkout();};
    rmv.appendChild(rb);
    const hd=el('div','set-grid hd'); hd.innerHTML='<div>Set</div><div>Kg</div><div>Reps</div><div>RPE</div><div></div><div></div>';
    b.appendChild(hd);
    ex.sets.forEach((s,si)=>b.appendChild(renderSet(ex,ei,s,si)));
    const er=el('div','e1rm'); er.id=`e1-${ei}`; er.innerHTML=bestE1(ex); b.appendChild(er);
    const foot=el('div','ex-foot');
    const addSet=el('button','btn xs sec','+ Set'); addSet.onclick=()=>{
      const last=ex.sets[ex.sets.length-1]||{};
      ex.sets.push({weight:last.weight||'',reps:'',rpe:'',warmup:false,done:false}); renderWorkout();};
    const warm=el('button','btn xs sec','+ Warmup'); warm.onclick=()=>{
      ex.sets.unshift({weight:'',reps:'',rpe:'',warmup:true,done:false}); renderWorkout();};
    foot.appendChild(addSet); foot.appendChild(warm);
    const restEdit=el('div','rest-edit');
    restEdit.appendChild(el('span','lbl','Rest'));
    const dn=el('button','step','−');
    const rv=el('span','rest-val'); rv.id=`rv-${ei}`; rv.textContent=(ex.rest_seconds||0)+'s';
    const up=el('button','step','+');
    const bump=d=>{ ex.rest_seconds=Math.max(0,(ex.rest_seconds||0)+d); rv.textContent=ex.rest_seconds+'s';
      saveRestPref(ex.exercise_id, ex.rest_seconds); };
    dn.onclick=()=>bump(-15); up.onclick=()=>bump(15);
    restEdit.appendChild(dn); restEdit.appendChild(rv); restEdit.appendChild(up);
    foot.appendChild(restEdit);
    b.appendChild(foot);
    return b;
  }
  function renderSet(ex,ei,s,si){
    const r=el('div','set-grid set-row'+(s.done?' done':'')+(s.sug&&!s.done?' sug':''));
    const no=el('div','setno'+(s.warmup?' warm':''), s.warmup?'W':String(workingIndex(ex,si)));
    const ph=k=> s.sug&&s.sug[k]!=null?String(s.sug[k]):'–';
    const w=el('input'); w.type='number'; w.inputMode='decimal'; w.placeholder=ph('weight'); w.value=s.weight;
    const rp=el('input'); rp.type='number'; rp.inputMode='numeric'; rp.placeholder=ph('reps'); rp.value=s.reps;
    const rpe=el('input'); rpe.type='number'; rpe.inputMode='decimal'; rpe.placeholder=ph('rpe'); rpe.value=s.rpe; rpe.step='0.5';
    w.oninput=()=>{s.weight=w.value;upd(ei);}; rp.oninput=()=>{s.reps=rp.value;upd(ei);}; rpe.oninput=()=>{s.rpe=rpe.value;};
    const chk=el('div','chk'+(s.done?' on':''),'✓');
    chk.onclick=()=>{ s.done=!s.done; r.classList.toggle('done',s.done); chk.classList.toggle('on',s.done);
      if(s.done){ startRest(ex.rest_seconds||120, ei); if(navigator.vibrate)navigator.vibrate(15);} };
    const del=el('button','set-del','✕'); del.title='Remove set';
    del.onclick=()=>{ ex.sets.splice(si,1); renderWorkout(); };
    r.appendChild(no);r.appendChild(w);r.appendChild(rp);r.appendChild(rpe);r.appendChild(chk);r.appendChild(del);
    return r;
  }
  function workingIndex(ex,si){ let n=0; for(let i=0;i<=si;i++){ if(!ex.sets[i].warmup) n++; } return n; }
  function upd(ei){ const e=$(`#e1-${ei}`); if(e) e.innerHTML=bestE1(active.exercises[ei]); }
  function bestE1(ex){ let best=0; ex.sets.forEach(s=>{const v=e1rm(parseFloat(s.weight),parseInt(s.reps)); if(v>best)best=v;});
    return best?`Best est. 1RM this session: <b style="color:var(--txt)">${best} kg</b>`:'Est. 1RM appears as you log'; }
  function lastLine(last){
    const d=last.date?new Date(last.date).toLocaleDateString(undefined,{month:'short',day:'numeric'}):'';
    const sets=last.sets.slice(0,4).map(x=>`${x.w!=null?x.w:'–'}×${x.reps!=null?x.reps:'–'}${x.rpe!=null?'@'+x.rpe:''}`).join(', ');
    return `Last${d?' ('+d+')':''}: ${sets}`;
  }
  let _tick=null;
  function tickElapsed(){ const e=$('#elapsed'); if(!e||!active)return;
    const sec=Math.floor((Date.now()-active.started)/1000); const m=Math.floor(sec/60);
    e.textContent=`${m}:${String(sec%60).padStart(2,'0')} elapsed`;
    clearTimeout(_tick); _tick=setTimeout(tickElapsed,1000);
  }

  /* ---------- rest timer ---------- */
  function startRest(sec,ei){ restRemain=sec; restEi=(ei==null?null:ei); $('#restBar').classList.remove('hidden'); paintRest();
    clearInterval(restTimer); restTimer=setInterval(()=>{ restRemain--; paintRest();
      if(restRemain<=0){ clearInterval(restTimer); $('#restBar').classList.add('hidden'); if(navigator.vibrate)navigator.vibrate([60,40,60]); } },1000); }
  function paintRest(){ const t=$('#restT'); if(!t)return; const m=Math.floor(Math.max(restRemain,0)/60); t.textContent=`${m}:${String(Math.max(restRemain,0)%60).padStart(2,'0')}`; }
  function adjustRest(delta){
    restRemain=Math.max(0,restRemain+delta); paintRest();
    if(restEi!=null && active){ const ex=active.exercises[restEi];
      if(ex){ ex.rest_seconds=Math.max(0,(ex.rest_seconds||0)+delta);
        const rv=$(`#rv-${restEi}`); if(rv) rv.textContent=ex.rest_seconds+'s';
        saveRestPref(ex.exercise_id, ex.rest_seconds); } }
  }
  $('#rest30').onclick=()=>adjustRest(30);
  $('#restMinus').onclick=()=>adjustRest(-30);
  $('#restSkip').onclick=()=>{clearInterval(restTimer);$('#restBar').classList.add('hidden');};
  ...
  async function finishWorkout(btn){
    ...
    active.exercises.forEach(ex=>{
      ex.sets.forEach((s,i)=>{
        const reps=parseInt(s.reps), wt=s.weight===''?null:parseFloat(s.weight);
        if(!s.done && !reps) return;
        rows.push({ workout_id:active.id, exercise_id:ex.exercise_id, exercise_name:ex.exercise_name,
          set_index:i+1, weight_kg:wt, reps:isNaN(reps)?null:reps,
          rpe:s.rpe===''?null:parseFloat(s.rpe), is_warmup:!!s.warmup, completed:!!s.done });
      });
    });
    if(!rows.length){ toast('Log at least one set first'); return; }
    const srpe=prompt('Session RPE (1–10)?  How hard was the whole session?','7');
    if(btn){btn.textContent='Saving…';}
    const {error:e1}=await sb.from('strength_sets').insert(rows);
    if(e1){ toast('Save failed: '+e1.message); if(btn)btn.textContent='Finish & save workout'; return; }
    await sb.from('strength_workouts').update({ status:'completed', completed_at:new Date().toISOString(),
      srpe: srpe?parseInt(srpe):null }).eq('id',active.id);
    if(active.assignment){ await sb.from('workout_assignments').update({status:'completed'}).eq('id',active.assignment.id); }
    toast('Workout saved 💪'); active=null; show('home');
  }
  async function discardWorkout(){
    if(!confirm('Discard this workout? Nothing will be saved.')) return;
    await sb.from('strength_workouts').delete().eq('id',active.id);
    if(active.assignment){ await sb.from('workout_assignments').update({status:'pending',workout_id:null}).eq('id',active.assignment.id); }
    active=null; toast('Discarded'); show('home');
  }

  /* ---------- EXERCISE PICKER ---------- */
  let pickFilter={q:'',muscle:'',equipment:''};
  const MUSCLES=['quadriceps','hamstrings','glutes','calves','chest','back','lats','shoulders','biceps','triceps','abdominals','forearms'];
  function openPicker(){
    ...
    bg.appendChild(sh); $('#sheet').innerHTML=''; $('#sheet').appendChild(bg);
    $('#pkClose').onclick=closeSheet;
    const chips=$('#pkChips');
    ['All',...MUSCLES].forEach(m=>{ const c=el('button','chip'+((m==='All'&&!pickFilter.muscle)?' on':''),cap(m));
      c.onclick=()=>{pickFilter.muscle=(m==='All'?'':m);[...chips.children].forEach(x=>x.classList.remove('on'));c.classList.add('on');runSearch();};
      chips.appendChild(c); });
    const si=$('#pkSearch'); si.oninput=()=>{pickFilter.q=si.value;clearTimeout(si._t);si._t=setTimeout(runSearch,220);};
    si.focus(); runSearch();
  }
  function closeSheet(){ if(_demoTimer){clearInterval(_demoTimer);_demoTimer=null;} const s=$('#sheet'); if(s) s.innerHTML=''; }
  async function runSearch(){
    let q=sb.from('exercises').select('id,name,equipment,primary_muscles,category').limit(60).order('name');
    if(pickFilter.q) q=q.ilike('name','%'+pickFilter.q+'%');
    if(pickFilter.muscle) q=q.contains('primary_muscles',[pickFilter.muscle]);
    const {data,error}=await q;
    const list=$('#pkList'); if(!list)return;
    if(error){list.innerHTML='<div class="empty">'+esc(error.message)+'</div>';return;}
    if(!data.length){list.innerHTML='<div class="empty">No matches.</div>';return;}
    list.innerHTML='';
    data.forEach(x=>{
  ...
  function addExercise(x){
    active.exercises.push({ exercise_id:x.id, exercise_name:x.name, rest_seconds:restPref(x.id,120),
      target_sets:null,target_reps:null,target_rpe:null,notes:null,
      sets:[{weight:'',reps:'',rpe:'',warmup:false,done:false},
            {weight:'',reps:'',rpe:'',warmup:false,done:false},
            {weight:'',reps:'',rpe:'',warmup:false,done:false}] });
    closeSheet(); renderWorkout(); toast(x.name+' added');
  }

  /* ---------- EXERCISE DEMO (heatmap + movement) ---------- */
  const REGIONS=new Set(['shoulders','chest','abdominals','biceps','triceps','forearms',
    'lats','back','traps','quadriceps','hamstrings','glutes','calves']);
  function normalizeMuscle(name){
    if(!name) return null; const k=String(name).trim().toLowerCase();
    const MAP={'abdominals':'abdominals','abs':'abdominals','biceps':'biceps','triceps':'triceps',
      'forearms':'forearms','chest':'chest','pectorals':'chest','shoulders':'shoulders',
      'deltoids':'shoulders','delts':'shoulders','lats':'lats','latissimus dorsi':'lats',
      'middle back':'back','lower back':'back','spine':'back','erector spinae':'back',
      'traps':'traps','trapezius':'traps','neck':'traps','quadriceps':'quadriceps','quads':'quadriceps',
      'adductors':'quadriceps','hamstrings':'hamstrings','glutes':'glutes','abductors':'glutes','calves':'calves'};
  ...
  function applyHeatmap(rootEl,primary,secondary){
    ...
    rootEl.querySelectorAll('.muscle').forEach(n=>{ const m=n.getAttribute('data-muscle');
      n.setAttribute('fill', prim.has(m)?'#ef4444':sec.has(m)?'#f59e0b':'#46525f'); });
  }
  function heatmapSVG(){
    const M=(d,a)=>`<g>${a.map(s=>`<${s.t} class="muscle" data-muscle="${d}" fill="#46525f" ${s.a}/>`).join('')}</g>`;
    return `<svg viewBox="0 0 470 426" xmlns="http://www.w3.org/2000/svg" role="img" aria-label="Muscles worked">
      ...
      ${M('shoulders',[{t:'ellipse',a:'cx="86" cy="90" rx="16" ry="14"'},{t:'ellipse',a:'cx="154" cy="90" rx="16" ry="14"'}])}
      ${M('chest',[{t:'ellipse',a:'cx="105" cy="114" rx="17" ry="13"'},{t:'ellipse',a:'cx="135" cy="114" rx="17" ry="13"'}])}
      ${M('abdominals',[{t:'rect',a:'x="104" y="130" width="32" height="56" rx="9"'}])}
      ${M('biceps',[{t:'ellipse',a:'cx="72" cy="128" rx="11" ry="23"'},{t:'ellipse',a:'cx="168" cy="128" rx="11" ry="23"'}])}
      ${M('forearms',[{t:'ellipse',a:'cx="64" cy="178" rx="10" ry="24"'},{t:'ellipse',a:'cx="176" cy="178" rx="10" ry="24"'}])}
      ${M('quadriceps',[{t:'ellipse',a:'cx="104" cy="258" rx="16" ry="44"'},{t:'ellipse',a:'cx="136" cy="258" rx="16" ry="44"'}])}
      ${M('calves',[{t:'ellipse',a:'cx="105" cy="350" rx="13" ry="34"'},{t:'ellipse',a:'cx="135" cy="350" rx="13" ry="34"'}])}
      ${M('shoulders',[{t:'ellipse',a:'cx="306" cy="90" rx="15" ry="13"'},{t:'ellipse',a:'cx="374" cy="90" rx="15" ry="13"'}])}
      ${M('traps',[{t:'path',a:'d="M322 76 q18 -12 36 0 l-6 22 q-12 6 -24 0 z"'}])}
      ${M('lats',[{t:'ellipse',a:'cx="316" cy="130" rx="13" ry="22"'},{t:'ellipse',a:'cx="364" cy="130" rx="13" ry="22"'}])}
      ${M('back',[{t:'rect',a:'x="328" y="118" width="24" height="70" rx="8"'}])}
      ${M('triceps',[{t:'ellipse',a:'cx="292" cy="128" rx="11" ry="23"'},{t:'ellipse',a:'cx="388" cy="128" rx="11" ry="23"'}])}
      ${M('forearms',[{t:'ellipse',a:'cx="284" cy="178" rx="10" ry="24"'},{t:'ellipse',a:'cx="396" cy="178" rx="10" ry="24"'}])}
      ${M('glutes',[{t:'ellipse',a:'cx="326" cy="220" rx="15" ry="16"'},{t:'ellipse',a:'cx="354" cy="220" rx="15" ry="16"'}])}
      ${M('hamstrings',[{t:'ellipse',a:'cx="324" cy="276" rx="14" ry="38"'},{t:'ellipse',a:'cx="356" cy="276" rx="14" ry="38"'}])}
      ${M('calves',[{t:'ellipse',a:'cx="324" cy="350" rx="13" ry="34"'},{t:'ellipse',a:'cx="356" cy="350" rx="13" ry="34"'}])}
    </svg>`;
  }
  const _mediaCache={};
  let _demoTimer=null;
  ...
  async function fetchExerciseMedia(id){
    ...
    const {data}=await sb.from('exercise_with_media').select('*').eq('id',id).maybeSingle();
    _mediaCache[id]=data||null; return _mediaCache[id];
  }
  async function openExerciseDemo(id,name){
    const bg=el('div','sheet-bg'); bg.onclick=e=>{if(e.target===bg)closeSheet();};
    const sh=el('div','sheet');
    ...
    const m=await fetchExerciseMedia(id); const body=$('#dmBody'); if(!body) return;
    if(!m){ body.innerHTML='<div class="empty">No library data for this exercise.</div>'; return; }
    const tag=a=>(a&&a.length)?a.map(x=>`<span class="demo-tag">${esc(x)}</span>`).join(''):'<span style="opacity:.5">—</span>';
    body.innerHTML=`
  ...
  function renderMovement(host,m){
    ...
    if(m.media_tier==='video' && angles.length){
      ...
      vid.loop=true; vid.muted=true; vid.playsInline=true; vid.autoplay=true; vid.setAttribute('preload','metadata');
      if(m.poster_front_url) vid.poster=m.poster_front_url;
      vid.src=angles[0].url; host.appendChild(vid);
      const ctr=el('div','demo-controls');
      if(angles.length>1){
        const seg=el('div','seg');
        angles.forEach((a,i)=>{ const b=el('button',i===0?'on':'',a.k);
          b.onclick=()=>{ if(i===cur)return; const t=vid.currentTime; cur=i; vid.src=angles[i].url;
            vid.onloadedmetadata=()=>{ try{vid.currentTime=t;}catch(e){} vid.playbackRate=rate; vid.play(); };
            [...seg.children].forEach(x=>x.classList.remove('on')); b.classList.add('on'); };
          seg.appendChild(b); });
        ctr.appendChild(el('span','lbl','Angle')); ctr.appendChild(seg);
      }
      const spd=el('div','seg');
      [['1×',1],['0.5×',.5],['0.25×',.25]].forEach(([lbl,r])=>{ const b=el('button',r===1?'on':'',lbl);
        b.onclick=()=>{ rate=r; vid.playbackRate=r; [...spd.children].forEach(x=>x.classList.remove('on')); b.classList.add('on'); };
        spd.appendChild(b); });
      ctr.appendChild(el('span','lbl','Speed')); ctr.appendChild(spd);
      host.appendChild(ctr); vid.playbackRate=rate; return;
    }
    const frames=(m.images||[]).filter(Boolean).map(exImg);
    if(frames.length>=2){
      frames.forEach(u=>{ const i=new Image(); i.src=u; });
      let idx=0, playing=true, period=900;
      const img=el('img','demo-video'); img.alt='Exercise movement demonstration'; img.src=frames[0];
      host.appendChild(img);
      const showFrame=i=>{ idx=(i+frames.length)%frames.length; img.src=frames[idx]; };
      const run =()=>{ if(_demoTimer) clearInterval(_demoTimer);
        _demoTimer = playing ? setInterval(()=>showFrame(idx+1), period) : null; };
      const ctr=el('div','demo-controls');
      const playSeg=el('div','seg');
      const pb=el('button','on','⏸ Pause');
      pb.onclick=()=>{ playing=!playing; pb.textContent=playing?'⏸ Pause':'▶ Play';
        if(playing) run(); else if(_demoTimer){ clearInterval(_demoTimer); _demoTimer=null; } };
      playSeg.appendChild(pb); ctr.appendChild(playSeg);
      const spd=el('div','seg');
      [['1×',900],['0.5×',1800],['0.25×',3600]].forEach(([lbl,ms])=>{ const b=el('button',ms===900?'on':'',lbl);
        b.onclick=()=>{ period=ms; [...spd.children].forEach(x=>x.classList.remove('on')); b.classList.add('on');
          if(playing) run(); };
        spd.appendChild(b); });
      ctr.appendChild(el('span','lbl','Speed')); ctr.appendChild(spd);
      host.appendChild(ctr);
      host.appendChild(el('div','demo-cap','Animated start → end positions from your exercise library.'));
      run(); return;
    }
    if(frames.length===1){ host.innerHTML=`<img class="demo-video" src="${esc(frames[0])}" alt="Exercise demonstration">`; return; }
    host.innerHTML='<div class="demo-fallback">🎬 Demonstration coming soon.<br>The muscle map above is live from your library data.</div>';
  }

  /* ---------- HISTORY + PRs ---------- */
  async function loadHistory(){
    const v=$('#historyView'); v.innerHTML='<div class="empty"><span class="spin"></span></div>';
    const [{data:wk},{data:prs}] = await Promise.all([
      sb.from('strength_workouts').select('*').eq('status','completed').order('completed_at',{ascending:false}).limit(25),
      sb.from('exercise_prs').select('*').order('best_e1rm_kg',{ascending:false}).limit(12)
    ]);
    v.innerHTML='';
    ...
    } else { v.appendChild(el('div','empty','PRs appear once you complete sets.')); }
    v.appendChild(el('div','section-title','Recent sessions'));
    if(wk&&wk.length){
      wk.forEach(w=>{
        const c=el('div','card');
        const d=w.completed_at?new Date(w.completed_at).toLocaleDateString(undefined,{weekday:'short',month:'short',day:'numeric'}):w.workout_date;
    ...
    } else { v.appendChild(el('div','empty','No completed workouts yet. Your saved sessions will show here.')); }
  }
  ...
  (async ()=>{
    ...
    USER = user || null;
    if(!USER){ $('#homeView').innerHTML='<div class="empty">Sign in on the dashboard to log strength sessions.</div>'; return; }
    loadHome();
  })();

  /* ---------- cleanup on unmount ---------- */
  return ()=>{ clearInterval(restTimer); clearTimeout(_tick); if(_demoTimer) clearInterval(_demoTimer); };
}
...
export default function StrengthLogger(){
  ...
  return (
    <>
      <style>{CSS}</style>
      <div className="slog" ref={rootRef} dangerouslySetInnerHTML={{ __html: SKELETON }} />
    </>
  );
}
