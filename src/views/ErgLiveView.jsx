  ...
  return (
    <div style={{
      ...
      <div style={{ textAlign: "center" }}>
        <div style={{ fontSize: 11, letterSpacing: 3, color: C.muted, marginBottom: 8 }}>
          LIVE ROWING
        </div>
        <div style={{ fontSize: 36, fontWeight: 700, color: C.accent, fontFamily: "monospace" }}>
          C2 CONNECT
        </div>
      </div>
      ...
      <div style={{ textAlign: "center", fontSize: 12, color: C.muted, lineHeight: 1.8, maxWidth: 300 }}>
        Power on your PM5.<br />
        Tap Connect — your browser will<br />
        ask you to select the device.
      </div>

      {error && (
        <div style={{
          background: "#2a0a14", border: `1px solid ${C.err}`, borderRadius: 8,
          padding: "12px 16px", fontSize: 12, color: C.err, maxWidth: 360, textAlign: "center",
        }}>{error}</div>
      )}
      ...
      <button
        ...
        disabled={connecting}
        style={{
          padding: "16px 48px", fontSize: 14, fontWeight: 700, letterSpacing: 2,
          background: connecting ? C.border : C.accent, color: C.bg,
          border: "none", borderRadius: 10, cursor: connecting ? "default" : "pointer",
          fontFamily: "inherit",
        }}
      >
    ...
    <div style={{
      ...
      <div style={{
        ...
        <div style={{
          ...
          <span style={{ fontSize: 10, letterSpacing: 2, color: isRowing ? C.accent : C.muted }}>
        ...
        <button
          ...
          style={{
            ...
            background: "transparent", border: `1px solid ${C.err}`, borderRadius: 6,
            color: C.err, cursor: "pointer", fontFamily: "inherit",
          }}
        >
          END
        </button>
      ...
      <div style={{
        ...
        <div style={{
          ...
          gap: 24, width: "100%", maxWidth: 380,
        }}>
          <LiveMetric label="WATTS"  value={metrics?.watts}      unit="W"   accent={C.gold}   dimmed={!isRowing} />
          <LiveMetric label="SPM"    value={metrics?.strokeRate}  unit="spm" accent={C.accent} dimmed={!isRowing} />
          <LiveMetric label="DIST"   value={metrics?.distance}    unit="m"   accent={C.cyan}   dimmed={!isRowing} />
        </div>
...
function SummaryScreen({ summary, onSave, onDiscard, saving }) {
  ...
  return (
    <div style={{
      ...
      <div style={{ fontSize: 11, letterSpacing: 3, color: C.muted }}>SESSION COMPLETE</div>
      ...
      <div style={{
        ...
        <LiveMetric label="DISTANCE" value={summary.distance}    unit="m"     accent={C.cyan}   size="normal" />
        <LiveMetric label="TIME"     value={summary.elapsedStr}  accent="#a0a0b8"               size="normal" />
        <LiveMetric label="AVG PACE" value={summary.avgPace ? parsePace(summary.avgPace) : "--"} unit="/500m" accent={C.cyan} size="normal" />
        <LiveMetric label="AVG WATT" value={summary.avgWatts}    unit="W"     accent={C.gold}   size="normal" />
        <LiveMetric label="AVG SPM"  value={summary.avgSpm}      unit="spm"   accent={C.accent} size="normal" />
        <LiveMetric label="CALORIES" value={summary.calories}    unit="kcal"  accent="#7e7e9a"  size="normal" />
      </div>

      {/* sRPE */}
      <div>
        <div style={{ fontSize: 9, letterSpacing: 2, color: C.muted, marginBottom: 8 }}>
      ...
      {/* Notes */}
      <div>
        <div style={{ fontSize: 9, letterSpacing: 2, color: C.muted, marginBottom: 6 }}>
          NOTES (optional)
        </div>
        <textarea
          value={notes}
          ...
          style={{
            ...
            color: C.text, fontFamily: "inherit", fontSize: 12, padding: "10px 12px",
            resize: "vertical",
          }}
        />
      </div>
      ...
      <div style={{ display: "flex", gap: 12 }}>
        <button
          ...
          disabled={saving}
          style={{
            flex: 1, padding: "14px", fontSize: 13, fontWeight: 700, letterSpacing: 1,
            background: saving ? C.border : C.accent, color: C.bg,
        ...
        <button
          onClick={onDiscard}
          style={{
            padding: "14px 20px", fontSize: 12, letterSpacing: 1,
...
export default function ErgLiveView({ plannedSessions = [], onSessionSaved }) {
  ...
  const todayStr = new Date().toISOString().slice(0, 10);
  const todaySession = plannedSessions.find(s => s.date === todayStr && s.type === "erg") || null;
